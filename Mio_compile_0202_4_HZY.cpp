...
void jmp() {jph=sduc[1];indeex=jpd[jph];}
void _if() {a=gein(sduc[1].c_str());ram[a]&1?:indeex++;}
void ext() {exit(0);}
...
void _for(){
	...
	indeex++;
	const int temp=indeex;
	for(ram[a]=ram[b];ram[a]<=ram[c];ram[a]++){
		while(1){
			sduc=opencode(code[indeex]);
			if(sduc[0]=="end"){if(ram[a]<ram[c]) indeex=temp;break;}
			runcode(sduc);
			indeex++;
		}
	}
...
void _while(){
	...
	while(ram[a]){
		while(1){
			sduc=opencode(code[indeex]);
			if(sduc[0]=="end"){if(ram[a]) indeex=temp;break;}
			runcode(sduc);
			indeex++;
		}
	}
...
void def(){
	string name=sduc[1];funname[name]=indeex;
...
int main() {
	...
	code.push_back("ext");
	for(; indeex<(int)code.size(); indeex++) runcode(sduc=opencode(code[indeex]));
	return 0;
}
