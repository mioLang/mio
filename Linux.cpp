...
void runcode(vector<string>&sduc){
	...
	try{
		...
		cout<<"Catch a error on line "<<line_index<<":"<<e.what()<<'\n';
	}catch (...){
		cout<<"On line"<<line_index<<",have unknown error\n"<<'\n';
	}
}
...
void jmp() {jph=sduc[1];line_index=jpd[jph];}
void _if() {a=gein(sduc[1].c_str());ram[a]&1?:line_index++;}
void ext() {exit(0);}
...
void _for(){
	...
	line_index++;
	const int temp=line_index;
	for(ram[a]=ram[b];ram[a]<=ram[c];ram[a]++){
		while(1){
			sduc=opencode(code[line_index]);
			if(sduc[0]=="end"){if(ram[a]<ram[c]) line_index=temp;break;}
			runcode(sduc);
			line_index++;
		}
	}
...
void _while(){
	...
	while(ram[a]){
		while(1){
			sduc=opencode(code[line_index]);
			if(sduc[0]=="end"){if(ram[a]) line_index=temp;break;}
			runcode(sduc);
			line_index++;
		}
	}
...
void def(){
	string name=sduc[1];funname[name]=line_index;
...
int main(int argc, char* argv[]) {
	...
	for(; line_index<(int)code.size(); line_index++) runcode(sduc=opencode(code[line_index]));
	return 0;
}
